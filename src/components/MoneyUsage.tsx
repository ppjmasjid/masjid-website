...
export default function MoneyUsage() {
    ...
    const handleSubmit = async () => {
        if (!selectedCategory || !whereUsed || !amount) {
        ...
        if (editingId) {
            const confirmInput = prompt(
                `To confirm the edit, type the category name exactly: "${selectedCategory}"`
            );
            if (confirmInput !== selectedCategory) {
                alert('Category name did not match. Edit cancelled.');
                return;
            }
        } else {
            ...
            const available = balanceMap[selectedCategory] || 0;
            if (amount > available) {
                alert('Not sufficient balance in this category.');
                return;
            }
        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-500">
                    <div>
                        ...
                        <p className="text-sm mt-1 text-gray-600">
                            Available:{' '}
                            <span className={(balanceMap[selectedCategory] || 0) <= 0 ? 'text-red-600' : 'text-green-600'}>
                                ৳{selectedCategory ? balanceMap[selectedCategory] || 0 : totalBalance}
                            </span>
                        </p>
                    </div>
